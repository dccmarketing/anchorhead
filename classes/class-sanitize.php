...
class Anchorhead_Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'anchorhead' ) );
		 ...
		 */
		do_action( 'anchorhead_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'anchorhead_post_sanitize', $sanitized );
